...
namespace gerber_3d
{
    ...
    void boundary_tesselator::on_combine_boundary(GLdouble vertex[3], GLvoid **dataOut)
    {
        ...
        fat_point &v = fat_points.emplace_back(x, y, 0);
    ...
    void boundary_tesselator::new_entity(int entity_id, uint32_t flags)
    {
        ...
        entities.emplace_back(entity_id, (int)boundaries.size(), (int)fills.size(), flags);
    ...
    void boundary_tesselator::pick_entities(vec2d const &world_pos, std::list<tesselator_entity const *> &picked)
    {
        ...
        for(auto const &e : entities) {
            if(e.bounds.contains(world_pos)) {
                picked.push_back(&e);
            }
        }
    }
