...
namespace
{
    ...
    gerber_error_code calculate_arc_sq(gerber_net *net, bool is_clockwise, vec2d const &center)
    {
        ...
        std::array<vec2d, 4> centers{ vec2d{ net->start.x + center.x, net->start.y + center.y }, vec2d{ net->start.x + center.x, net->start.y - center.y },
                                      vec2d{ net->start.x - center.x, net->start.y + center.y }, vec2d{ net->start.x - center.x, net->start.y - center.y } };
        ...
        for(auto const &c : centers) {
            ...
            // YOINK not identical to C# Math.Round(double, int) ...
            vec2d c2{ round_precise(c.x, 6), round_precise(c.y, 6) };
    ...
    static void update_bounds(rect &bounds, matrix const &matrix, vec2d const &point)
    {
    ...
    template <typename T> static void update_bounds(rect &bounds, matrix const &matrix, T points)
    {
...
namespace gerber_lib
{
    ...
    gerber_error_code gerber::parse_rs274x(gerber_net *net)
    {
        ...
        case 'AD': {
            ...
            if(parse_aperture_definition(aperture, &image, unit_scale, &aperture_number) == ok) {

                if(aperture_number >= 0 && aperture_number <= max_num_apertures) {

                    aperture->unit = state.net_state->unit;

                    if(image.apertures.find(aperture_number) != image.apertures.end()) {
        ...
        case 'IJ': {
            ...
            LOG_DEBUG("Image justify: A:{} (offset {}) B:{} (offset {})", image.info.justify_a, image.info.image_justify_offset_a, image.info.justify_b,
        ...
        case 'SR': {
            ...
            LOG_DEBUG("Step and repeat: POS: {},{}, DISTANCE: {},{}", state.level->step_and_repeat.pos.x, state.level->step_and_repeat.pos.y,
                      state.level->step_and_repeat.distance.x, state.level->step_and_repeat.distance.y);
        } break;
        ...
        while(!reader.eof() && !done) {
            ...
            case 'X': {
                ...
                if(image.format.omit_zeros == omit_zeros_trailing) {
                    add_trailing_zeros(image.format.integral_part_x, image.format.decimal_part_x, (int)length, &coordinate);
                }
            ...
            case 'Y': {
                ...
                CHECK(reader.get_int(&coordinate, &length));
                if(image.format.omit_zeros == omit_zeros_trailing) {
                    add_trailing_zeros(image.format.integral_part_x, image.format.decimal_part_y, (int)length, &coordinate);
                }
                if(image.format.coordinate == coordinate_incremental) {
                    if(coordinate != 0) {
            ...
            case 'J': {
                ...
                CHECK(reader.get_int(&coordinate, &length));
                if(image.format.omit_zeros == omit_zeros_trailing) {
                    add_trailing_zeros(image.format.integral_part_y, image.format.decimal_part_y, (int)length, &coordinate);
                }
    ...
    gerber_error_code gerber::draw_arc(gerber_draw_interface &drawer, gerber_arc const &arc, double thickness, gerber_polarity polarity) const
    {
        ...
        auto add_arc = [&](vec2d const &pos, double radius, double s, double e) {
            draw_elements[n] = gerber_draw_element(pos, s, e, radius);
            n += 1;
        };
    ...
    gerber_error_code gerber::draw(gerber_draw_interface &drawer, int const hide_elements, int start_net_index, int end_net_index) const
    {
        ...
        for(size_t net_index = 0; net_index < image.nets.size(); net_index = next_net_index(net_index)) {
            ...
            gerber_net *n = image.nets[net_index];
            ...
            if(n->aperture_state == aperture_state_off) {
                continue;
            }
            ...
            map_get_if_found(image.apertures, n->aperture, &aperture);
            ...
            case interpolation_region_start: {
                ...
                if(!should_hide(hide_element_outlines)) {
                    CHECK(fill_region_path(drawer, net_index, n->level->polarity));
                }
                    ...
                    case aperture_state_flash: {
                        ...
                        case aperture_type_circle: {
                            ...
                            if(!should_hide(hide_element_circles)) {
                                ...
                                CHECK(draw_circle(drawer, n->start, radius, n->level->polarity));
                        ...
                        case aperture_type_oval: {
                            ...
                            if(!should_hide(hide_element_ovals)) {
                                ...
                                CHECK(draw_capsule(drawer, n->start, w, h, n->level->polarity));
                        ...
                        case aperture_type_polygon: {
                            ...
                            if(!should_hide(hide_element_polygons)) {
                                ...
                                double p0 = (float)aperture->parameters[0];
                                double p1 = (float)aperture->parameters[1];
                                double p2 = (float)aperture->parameters[2];
                        ...
                        case aperture_type_macro: {
                            ...
                            if(!should_hide(hide_element_macros)) {
                                CHECK(draw_macro(drawer, n, aperture));
                            }
                        } break;
                        ...
                        case interpolation_linear:
                            if(aperture->parameters.size() < 1) {
                                ...
                                if(!should_hide(hide_element_lines)) {
                                    // gerber_draw_element e;
                                    // e.draw_element_type = draw_element_line;
                                    // e.line_start = n->start;
                                    // e.line_end = n->end;
                                    // drawer.draw_element(e, aperture->parameters[0], n->level->polarity);
                                    CHECK(draw_linear_track(drawer, n->start, n->end, aperture->parameters[0], n->level->polarity));
                                }
                            }
                            break;
                        ...
                        case interpolation_counterclockwise_circular:
                            if(aperture->parameters.size() < 1) {
                                ...
                                if(!should_hide(hide_element_arcs)) {
                                    ...
                                    CHECK(draw_arc(drawer, n->circle_segment, aperture->parameters[0], n->level->polarity));
                                    // gerber_draw_element e;
                                    // e.draw_element_type = draw_element_arc;
                                    // e.arc_center = n->circle_segment.pos;
